...
class AutoDistributionService extends events_1.EventEmitter {
    constructor() {
        super();
        this.isRunning = false;
        this.checkInterval = 300000; // 5 минут (300 секунд)
    ...
    async updateContractBalance(contract) {
    ...
    async getSolanaContractBalance(contract) {
        try {
            const connection = new web3_js_1.Connection(this.config.solana.rpcUrl, 'confirmed');
            const vestingPDA = new web3_js_1.PublicKey(contract.address);
    ...
    async getBNBContractBalance(contract) {
        try {
            const provider = new ethers_1.ethers.JsonRpcProvider(this.config.bnb.rpcUrl);
            const vestingABI = [
            ...
            const vestingContract = new ethers_1.ethers.Contract(contract.address, vestingABI, provider);
    ...
    async shouldDistribute(contract, currentTime) {
        ...
        for (const period of unlockPeriods) {
            console.log(`   - Period ${period.percentage}%: ${elapsed >= period.time ?
                '✅ UNLOCKED' : '⏳ PENDING'} (${Math.floor(period.time / 60)}min) ${contract.distributedPeriods.has(period.percentage) ? '[DISTRIBUTED]' : '[WAITING]'}`);
    ...
    calculateVestedAmount(totalAmount, elapsedSeconds) {
        let percentage;
        if (elapsedSeconds < 0) {
            percentage = 0;
        }
    ...
    // Выполнение раздачи
    async distribute(contract, period) {
        try {
            let result;
            if (contract.chain === 'solana') {
    ...
    async distributeSolana(contract, period) {
        console.log(`🌞 Starting Solana distribution for period ${period}% using 3-claim.js logic...`);
        const connection = new web3_js_1.Connection(this.config.solana.rpcUrl, 'confirmed');
        const programId = new web3_js_1.PublicKey(this.config.solana.programId);
        const vestingPDA = new web3_js_1.PublicKey(contract.address);
        ...
        const vestingAccount = this.parseVestingAccount(vestingAccountInfo.data);
        const mintAddress = new web3_js_1.PublicKey(vestingAccount.mint);
        ...
        const initializer = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(privateKeyArray));
        ...
        try {
            ...
            // Получаем vault PDA
            const [vaultPDA] = await web3_js_1.PublicKey.findProgramAddress([Buffer.from('vault'), vestingPDA.toBuffer()], programId);
            ...
            for (const recipient of contract.recipients) {
                ...
                const ata = await (0, spl_token_1.getAssociatedTokenAddress)(mintAddress, recipientPubkey);
                ...
                try {
                    ...
                    transaction.add((0, spl_token_1.createAssociatedTokenAccountInstruction)(initializer.publicKey, // payer
                    ata, // ata
            ...
            const [vaultAuthority] = await web3_js_1.PublicKey.findProgramAddress([Buffer.from('authority'), vestingPDA.toBuffer()], programId);
            // Создаем claim инструкцию точно как в 3-claim.js
            const claimInstruction = this.createClaimInstruction(programId, initializer.publicKey, // initializer (signer)
            ...
            const signature = await (0, web3_js_1.sendAndConfirmTransaction)(connection, transaction, [initializer], {
    ...
    async distributeBNB(contract, period) {
        console.log(`🟡 Starting BNB distribution for period ${period}% using ProductionTokenVesting logic...`);
        const provider = new ethers_1.ethers.JsonRpcProvider(this.config.bnb.rpcUrl);
        const wallet = new ethers_1.ethers.Wallet(this.config.bnb.privateKey, provider);
        ...
        try {
            ...
            console.log(`BNB balance: ${ethers_1.ethers.formatEther(balance)} BNB`);
            ...
            const vestingContract = new ethers_1.ethers.Contract(contract.address, vestingABI, wallet);
            ...
            console.log(`   Total amount: ${ethers_1.ethers.formatEther(schedule.totalAmount)} tokens`);
            ...
            console.log(`  - Total amount: ${ethers_1.ethers.formatEther(schedule.totalAmount)} tokens`);
            console.log(`  - Claimed amount: ${ethers_1.ethers.formatEther(schedule.claimedAmount)} tokens`);
            ...
            const claimableAmount = await vestingContract.getClaimableAmount(contract.beneficiaryAddress);
            console.log(`📋 Claimable amount: ${ethers_1.ethers.formatEther(claimableAmount)} tokens`);
            ...
            const distributionAmounts = recipients.map((recipient) => ({
    ...
    createClaimInstruction(programId, initializer, vestingPDA, vaultPDA, vaultAuthority, recipientATAs) {
    ...
    loadVestingContracts() {
        try {
            ...
            if (this.config.bnb.contractAddress && this.config.bnb.privateKey && this.config.bnb.knownInitializer) {
                ...
                try {
                    const wallet = new ethers_1.ethers.Wallet(this.config.bnb.privateKey);
    ...
    async loadSolanaContractData(contract) {
        try {
            const connection = new web3_js_1.Connection(this.config.solana.rpcUrl, 'confirmed');
            const vestingPDA = new web3_js_1.PublicKey(contract.address);
            ...
            if (totalClaimed > 0n) {
                ...
                if (claimedPercentage >= 9.5)
                    contract.distributedPeriods.add(10); // 10% ± погрешность
                if (claimedPercentage >= 19.5)
                    contract.distributedPeriods.add(20); // 20% ± погрешность  
                if (claimedPercentage >= 49.5)
                    contract.distributedPeriods.add(50); // 50% ± погрешность
                if (claimedPercentage >= 99.5)
                    contract.distributedPeriods.add(100); // 100% ± погрешность
    ...
    async loadBNBContractData(contract) {
        try {
            const provider = new ethers_1.ethers.JsonRpcProvider(this.config.bnb.rpcUrl);
            const vestingABI = [
                'function getVestingSchedule(address beneficiary) external view returns (bool isInitialized, address token, address authorizedFunder, uint256 startTime, uint256 cliffDuration, uint256 vestingDuration, uint256 totalAmount, uint256 claimedAmount, uint8 recipientCount)',
            ...
            console.log(`📋 Schedule found:`, {
                ...
                totalAmount: ethers_1.ethers.formatEther(schedule.totalAmount)
            });
            if (!schedule.isInitialized) {
            ...
            console.log(`   - Total amount: ${ethers_1.ethers.formatEther(schedule.totalAmount)} tokens`);
            console.log(`   - Claimed amount: ${ethers_1.ethers.formatEther(schedule.claimedAmount)} tokens`);
            console.log(`   - Claimable now: ${ethers_1.ethers.formatEther(claimableAmount)} tokens`);
            ...
            try {
                const recipients = await vestingContract.getRecipients(contract.beneficiaryAddress);
                contract.recipients = recipients.map((r) => ({
                    wallet: r.wallet,
    ...
    addVestingContract(contractData) {
        const id = `${contractData.chain}_${contractData.address}_${Date.now()}`;
        const contract = {
            ...contractData,
            id,
            distributedPeriods: contractData.distributedPeriods || new Set(),
    ...
    resetDistributionState(contractId) {
        const contract = this.vestingContracts.get(contractId);
        if (contract) {
            contract.distributedPeriods.clear();
...
exports.default = AutoDistributionService;
